...
// lightspeed: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-lightspeed-address
...
// lightspeed: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(ClightspeedAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(ClightspeedAddress(address).ToString());
}
...
// lightspeed: check 'spent' consistency between wallet and txindex
// lightspeed: fix wallet spent state according to txindex
...
// lightspeed: disable transaction (only for coinstake)

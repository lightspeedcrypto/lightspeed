...
{
    QString ret = QString("lightspeed:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LSD unit input in lnReqAmount, we generate the URI with LSD as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(lightspeedUnits::format(lightspeedUnits::LSD, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
